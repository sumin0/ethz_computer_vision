...
class Simulator(object):
	...
	def getMeasurementModel(self, locations):
		...
		one_vec = np.ones_like(locations)
		measurement = one_vec - (one_vec-door_1) * (one_vec-door_2) * (one_vec-door_3)
	...
	def moveParticles(self, particles, uk):
		...
		noise = scipy.stats.norm(0, self.param.odometry_stdev).rvs(size=len(particles))
		prior_particles = particles + uk*np.ones_like(particles) + noise
	...
	def measureParticles(self, prior_particles, zk):
		...
		if zk:
			...
			particle_weights = 1 - self.getMeasurementModel(prior_particles)
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		beta = 0
		resampled_particles = np.zeros(self.param.num_particles)
		w_max = max(particle_weights)
		index = np.random.randint(0, self.param.num_particles)
		for i in range(self.param.num_particles):
			...
			while particle_weights[index] < beta:
				...
				index = (index + 1) % self.param.num_particles
